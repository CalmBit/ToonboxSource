...
class DistributedPartyTrampolineActivityAI(DistributedPartyActivityAI, FSM):
    ...
    def awardBeans(self, numBeans, height):
        ...
        if avId != self.currentAv:
            self.air.writeServerEventMessage('suspicious',avId,'Tried to give beans while not using the trampoline!')
            return
        if self.state != 'Active':
            self.air.writeServerEventMessage('suspicious',avId,'Toon tried to award beans while the game wasn\'t running!')
            return
        if numBeans != self.collected:
            self.air.writeServerEventMessage('suspicious',avId,'Toon reported incorrect number of collected jellybeans!')
        av = self.air.doId2do.get(avId, None)
        if not av:
            self.air.writeServerEventMessage('suspicious',avId,'Toon tried to award beans while not in district!')
            return
    ...
    def reportHeightInformation(self, height):
        ...
        av = self.air.doId2do.get(avId, None)
        if not av:
            self.air.writeServerEventMessage('suspicious',avId,'Toon tried to report height without being on the district!')
            return
        if height > self.record:
            self.record = height
            self.sendUpdate('setBestHeightInfo', [av.getName(), height])
        else:
            self.air.writeServerEventMessage('suspicious',avId,'Toon incorrectly reported height!')
    ...
    def requestAnim(self, anim):
        avId = self.air.getAvatarIdFromSender()
        if self.state != 'Active':
            self.air.writeServerEventMessage('suspicious',avId,'Toon tried to request an animation while not playing!')
            return
        if self.currentAv != avId:
            self.air.writeServerEventMessage('suspicious',avId, 'Toon tried to request an anim for someone else!')
            return
    ...
    def removeBeans(self, beans):
        avId = self.air.getAvatarIdFromSender()
        if self.state != 'Active':
            self.air.writeServerEventMessage('suspicious',avId,'Toon tried to collect jellybeans while not playing!')
            return
        if self.currentAv != avId:
            self.air.writeServerEventMessage('suspicious',avId, 'Toon tried to collect jellybeans while someone else was playing!')
            return
        for bean in beans:
            if not bean in self.jellybeans:
                self.air.writeServerEventMessage('suspicious',avId,'Toon tried to collect non-existent bean!')
                beans.remove(bean)
            else:
                self.collected += 1
    ...
    def toonExitRequest(self):
        avId = self.air.getAvatarIdFromSender()
        if self.state != 'Active':
            self.air.writeServerEventMessage('suspicious',avId,'Toon tried to leave a trampoline that was not running!')
            return
        if self.currentAv != avId:  
            self.air.writeServerEventMessage('suspicious',avId,'Toon tried to exit trampoline for someone else!')
            return
    ...
    def toonExitDemand(self):
        avId = self.air.getAvatarIdFromSender()
        if avId != self.currentAv:
            self.air.writeServerEventMessage('suspicious',avId,'Toon tried to exit trampoline they\'re not using!')
            return
        self.demand('Idle')
    ...
    def toonReady(self):
        avId = self.air.getAvatarIdFromSender()
        if self.state != 'Rules':
            self.air.writeServerEventMessage('suspicious',avId,'Toon tried to verify rules while the rules were not running!')
            return
        if avId != self.currentAv:
            self.air.writeServerEventMessage('suspicious',avId,'Toon tried to verify rules for someone else!')
            return
        self.demand('Active')
