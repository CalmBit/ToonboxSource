...
class DistributedPartyDanceActivityBaseAI(DistributedPartyActivityAI):
    ...
    def updateDancingToon(self, state, anim):
        ...
        if not avId in self.toons:
            self.air.writeServerEventMessage('suspicious',avId,'Toon tried to update their state while not dancing!')
            return
    ...
    def toonJoinRequest(self):
        ...
        if avId in self.toons:
            self.air.writeServerEventMessage('suspicious',avId,'Toon tried to enter dance activity twice!')
            return
        av = self.air.doId2do.get(avId)
        if not av:
            self.air.writeServerEventMessage('suspicious',avId,'Toon tried to interact with a party activity from a different district!')
            return
    ...
    def toonExitRequest(self):
        avId = self.air.getAvatarIdFromSender()
        if not avId in self.toons:
            self.air.writeServerEventMessage('suspicious',avId,'Toon tried to exit a dance floor they\'re not on!')
            return
