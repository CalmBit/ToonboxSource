...
class DistributedPartyCannonActivityAI(DistributedPartyActivityAI):
    ...
    def setLanded(self, toonId):
        ...
        if avId != toonId:
            self.air.writeServerEventMessage('suspicious',avId,'Toon tried to land someone else!')
            return
        if not avId in self.toonsPlaying:
            self.air.writeServerEventMessage('suspicious',avId,'Toon tried to land while not playing the cannon activity!')
            return
    ...
    def requestCloudHit(self, cloudId, r, g, b):
        avId = self.air.getAvatarIdFromSender()
        if not avId in self.toonsPlaying:
            self.air.writeServerEventMessage('suspicious',avId,'Toon tried to hit cloud in cannon activity they\'re not using!')
            return
