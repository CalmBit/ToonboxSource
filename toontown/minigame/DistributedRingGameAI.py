...
class DistributedRingGameAI(DistributedMinigameAI):
    ...
    def setToonGotRing(self, success):
        ...
        if avId not in self.avIdList:
            self.air.writeServerEventMessage('suspicious', avId, 'RingGameAI.setToonGotRing: invalid avId')
            return
        if self.gameFSM.getCurrentState() is None or self.gameFSM.getCurrentState().getName() != 'swimming':
            self.air.writeServerEventMessage('suspicious', avId, 'RingGameAI.setToonGotRing: game not in swimming state')
            return
