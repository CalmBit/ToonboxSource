...
class DistributedDivingGameAI(DistributedMinigameAI):
    ...
    def treasureRecovered(self):
        ...
        if avId not in self.avIdList:
            self.air.writeServerEventMessage('suspicious', avId, 'DivingGameAI.treasureRecovered: invalid avId')
            return

        if avId not in self.treasureHolders:
            self.air.writeServerEventMessage('suspicious', avId, 'DivingGameAI.treasureRecovered: tried to recover without holding treasure')
            return
    ...
    def gameOver(self):
        ...
        self.air.writeServerEventMessage('MiniGame Stats', None, trackingString)
        return
    ...
    def pickupTreasure(self, chestId):
        ...
        avId = self.air.getAvatarIdFromSender()
        if avId not in self.avIdList:
            self.air.writeServerEventMessage('suspicious', avId, 'DivingGameAI.pickupTreasure: invalid avId')
            return
        if avId in self.treasureHolders:
            self.air.writeServerEventMessage('suspicious', avId, 'DivingGameAI.pickupTreasure: already holding treasure')
            return
        if not (0 <= chestId < len(self.treasureHolders)):
            self.air.writeServerEventMessage('suspicious', avId, 'DivingGameAI.pickupTreasure: invalid chest requested (#%d)' % chestId)
            return
    ...
    def handleCrabCollision(self, avId, status):
        if avId not in self.avIdList:
            self.air.writeServerEventMessage('suspicious', avId, 'DivingGameAI.handleCrabCollision: invalid avId')
            return
        timestamp = globalClockDelta.getFrameNetworkTime()
        ...
        if status == 'normal' or status == 'treasure':
            ...
            if status == 'treasure':
                if avId in self.treasureHolders:
                    ...
                    self.air.writeServerEventMessage('suspicious', avId, 'DivingGameAI.handleCrabCollision: reported "treasure drop" without holding treasure')

    def handleFishCollision(self, avId, spawnId, spawnerId, status):
        if avId not in self.avIdList:
            self.air.writeServerEventMessage('suspicious', avId, 'DivingGameAI.handleFishCollision: invalid avId')
            return
        timestamp = globalClockDelta.getFrameNetworkTime()
        self.sendUpdate('setTreasureDropped', [avId, timestamp])
        timestamp = globalClockDelta.getFrameNetworkTime()
        ...
        if status == 'treasure':
            if avId in self.treasureHolders:
                ...
                self.air.writeServerEventMessage('suspicious', avId, 'DivingGameAI.handleFishCollision: reported "treasure drop" without holding treasure')
