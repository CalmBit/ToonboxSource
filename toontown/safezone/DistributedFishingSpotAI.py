...
class DistributedFishingSpotAI(DistributedObjectAI):
    ...
    def requestEnter(self):
        ...
        if self.avId != None:
            if self.avId == avId:
                self.air.writeServerEventMessage('suspicious', avId, 'Toon requested to enter a pier twice!')
    ...
    def requestExit(self):
        avId = self.air.getAvatarIdFromSender()
        if self.avId != avId:
            self.air.writeServerEventMessage('suspicious', avId, 'Toon requested to exit a pier they\'re not on!')
            return
    ...
    def sellFish(self):
        avId = self.air.getAvatarIdFromSender()
        if self.avId != avId:
            self.air.writeServerEventMessage('suspicious', avId, 'Toon tried to sell fish at a pier they\'re not using!')
            return
        if self.air.doId2do[pondDoId].getArea() != ToontownGlobals.MyEstate:
            self.air.writeServerEventMessage('suspicious', avId, 'Toon tried to sell fish at a pier not in their estate!')
        av = self.air.doId2do[avId]
    ...
    def rewardIfValid(self, target):
        if not self.cast:
            self.air.writeServerEventMessage('suspicious', self.avId, 'Toon tried to fish without casting!')
            return
