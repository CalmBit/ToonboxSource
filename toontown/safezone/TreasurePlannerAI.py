...
class TreasurePlannerAI(DirectObject.DirectObject):
    ...
    def grabAttempt(self, avId, treasureId):
        if self.lastRequestId == avId:
            ...
            if elapsed > 10:
                ...
                if self.requestCount >= 3 and secondsPerGrab <= 0.4:
                    simbase.air.writeServerEventMessage('suspicious', avId, 'TreasurePlannerAI.grabAttempt %s treasures in %s seconds' % (self.requestCount, elapsed))
        else:
        ...
        if index == None:
            ...
            if av == None:
                simbase.air.writeServerEventMessage('suspicious', avId, 'TreasurePlannerAI.grabAttempt unknown avatar')
