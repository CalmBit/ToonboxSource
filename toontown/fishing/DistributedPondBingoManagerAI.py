...
class DistributedPondBingoManagerAI(DistributedObjectAI):
    ...
    def cardUpdate(self, cardId, cellId, genus, species):
        ...
        if not spot:
            self.air.writeServerEventMessage('suspicious', avId, 'Toon tried to call bingo while not fishing!')
            return
        fishTuple = (genus, species)
        if (genus != spot.lastFish[1] or species != spot.lastFish[2]) and (spot.lastFish[0] != FishGlobals.BootItem):
            self.air.writeServerEventMessage('suspicious', avId, 'Toon tried to update bingo card with a fish they didn\'t catch!')
            return
        if cardId != self.cardId:
            self.air.writeServerEventMessage('suspicious', avId, 'Toon tried to update expired bingo card!')
            return
        if self.state != 'Playing':
            self.air.writeServerEventMessage('suspicious', avId, 'Toon tried to update while the game is not running!')
            return
    ...
    def handleBingoCall(self, cardId):
        ...
        if not spot:
            ...
            return
        if not self.canCall:
            self.air.writeServerEventMessage('suspicious', avId, 'Toon tried to call bingo whle the game is not running!')
            return
        if cardId != self.cardId:
            self.air.writeServerEventMessage('suspicious', avId, 'Toon tried to call bingo with an expired cardId!')
            return
