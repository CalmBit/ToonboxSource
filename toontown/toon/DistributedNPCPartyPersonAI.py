...
class DistributedNPCPartyPersonAI(DistributedNPCToonBaseAI):
    ...
    def answer(self, wantsToPlan):
        avId = self.air.getAvatarIdFromSender()
        if self.busy != avId:
            self.air.writeServerEventMessage('suspicious', avId, 'DistributedNPCPartyPersonAI.answer busy with %s' % self.busy)
        ...
        if wantsToPlan:
            ...
            if av:
                if av.getGameAccess() != ToontownGlobals.AccessFull:
                    self.air.writeServerEventMessage('suspicious', avId, 'DistributedNPCPartyPersonAI.free player tried to host party.')
