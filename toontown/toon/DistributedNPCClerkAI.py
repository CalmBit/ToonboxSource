...
class DistributedNPCClerkAI(DistributedNPCToonBaseAI):
    ...
    def setInventory(self, blob, newMoney, done):
        ...
        if self.busy != avId:
            if self.busy != 0:
                self.air.writeServerEventMessage('suspicious', avId, 'DistributedNPCClerkAI.setInventory busy with %s' % self.busy)
        ...
        if avId in self.air.doId2do:
            ...
            if av.inventory.validatePurchase(newInventory, currentMoney, newMoney):
                ...
                self.air.writeServerEventMessage('suspicious', avId, 'DistributedNPCClerkAI.setInventory invalid purchase')
