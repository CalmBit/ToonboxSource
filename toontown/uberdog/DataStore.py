        ...
        if self.wantAnyDbm:
            ...
            if os.path.exists(self.filepath):
                try:
                    ...
                    uber.air.writeServerEventMessage('Uberdog data store Info', 0, 'Creating backup of file: %s saving as: %s' % (tail, newFileName))
                except:
                    uber.air.writeServerEventMessage('Uberdog data store Info', 0, 'Unable to create backup of file: %s ' % tail)
                ...
                for file in files:
                    if file.find(tail) > -1:
                        ...
                        try:
                            ...
                            uber.air.writeServerEventMessage('Uberdog data store Info', 0, 'Creating backup of file: %s saving as: %s' % (file, newFileName + ext))
                        except:
                            uber.air.writeServerEventMessage('Uberdog data store Info', 0, 'Unable to create backup of file: %s ' % newFileName + ext)
