...
class DistributedRaceAI(DistributedObjectAI, FSM):
    ...
    def hasGag(self, slot, requestedGag, index):
        ...
        if not avId in self.avatars:
            self.air.writeServerEventMessage('suspicious', avId, 'Toon tried to get gag in a race they\'re not in!')
            return
        if self.raceType == RaceGlobals.Practice:
            self.air.writeServerEventMessage('suspicious', avId, 'Toon tried to gain gag in a practice race!')
            return
        places = sorted(self.avatarProgress, key=self.avatarProgress.get)
        avPlace = places.index(avId)
        gag = self.gags[slot]
        if not gag[0]:
            self.air.writeServerEventMessage('suspicious', avId, 'Toon tried to pick up a gag that doesn\'t exist!')
            return
        gagIndex = gag[1]
        realGag = RaceGlobals.GagFreq[avPlace][gagIndex]
        if realGag != requestedGag:
            self.air.writeServerEventMessage('suspicious', avId, 'Toon tried to get the wrong gag!')
            return
        self.gags[slot] = [0, 0]
    ...
    def racerLeft(self, avId):
        ...
        if realAvId != avId:
            self.air.writeServerEventMessage('suspicious', realAvId, 'Toon tried to make another quit race!')
            return
        if not avId in self.avatars:
            self.air.writeServerEventMessage('suspicious', avId, 'Toon tried to leave race they\'re not in!')
            return
    ...
    def heresMyT(self, avId, laps, currentLapT, timestamp):
        ...
        if not avId == realAvId:
            self.air.writeServerEventMessage('suspicious', realAvId, 'Toon tried to send a message as another toon!')
            return
        if not avId in self.avatars:
            self.air.writeServerEventMessage('suspicious', avId, 'Toon not in race tried to send update to race!')
            return
    ...
    def avatarFinished(self, avId):
        if not avId in self.avatars:
            self.air.writeServerEventMessage('suspicious', avId, 'Toon tried to finish in a race they\'re not in!')
            return

        if avId in self.finishedAvatars:
            self.air.writeServerEventMessage('suspicious', avId, 'Toon tried to finish in a race twice!')
            return
    ...
    def requestThrow(self, x, y, z):
        avId = self.air.getAvatarIdFromSender()
        if not avId in self.avatars:
            self.air.writeServerEventMessage('suspicious', avId, 'Toon tried to throw a gag in a race they\'re not in!')
        if self.avatarGags[avId] == RaceGlobals.BANANA:
            ...
            self.air.writeServerEventMessage('suspicious', avId, 'Toon use race gag while not having one!')
    ...
    def requestKart(self):
        ...
        if not avId in self.avatars:
            self.air.writeServerEventMessage('suspicious', avId, 'Toon tried to request kart in race they\'re not in!')
            return
        for aK in self.avatarKarts:
            if aK[0] == avId:
