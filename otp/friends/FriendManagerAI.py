...
class FriendManagerAI(DistributedObjectAI):
    ...
    def friendQuery(self, requested):
        ...
        if not requested in self.air.doId2do:
            self.air.writeServerEventMessage('suspicious', avId, 'Player tried to friend a player that does not exist!')
            return
    ...
    def cancelFriendQuery(self, context):
        ...
        if not context in self.requests:
            self.air.writeServerEventMessage('suspicious', avId, 'Player tried to cancel a request that doesn\'t exist!')
            return
        if avId != self.requests[context][0][0]:
            self.air.writeServerEventMessage('suspicious', avId, 'Player tried to cancel someone elses request!')
            return
    ...
    def inviteeFriendConsidering(self, yesNo, context):
        avId = self.air.getAvatarIdFromSender()
        if not context in self.requests:
            self.air.writeServerEventMessage('suspicious', avId, 'Player tried to consider a friend request that doesn\'t exist!')
            return
        if avId != self.requests[context][0][1]:
            self.air.writeServerEventMessage('suspicious', avId, 'Player tried to consider for someone else!')
            return
        if self.requests[context][1] != 'friendQuery':
            self.air.writeServerEventMessage('suspicious', avId, 'Player tried to reconsider friend request!')
            return
        if yesNo != 1:
    ...
    def inviteeFriendResponse(self, response, context):
        avId = self.air.getAvatarIdFromSender()
        if not context in self.requests:
            self.air.writeServerEventMessage('suspicious', avId, 'Player tried to respond to a friend request that doesn\'t exist!')
            return
        if avId != self.requests[context][0][1]:
            self.air.writeServerEventMessage('suspicious', avId, 'Player tried to respond to someone else\'s request!')
            return
        if self.requests[context][1] == 'cancelled':
            self.air.writeServerEventMessage('suspicious', avId, 'Player tried to respond to non-active friend request!')
            return
    ...
    def inviteeAcknowledgeCancel(self, context):
        avId = self.air.getAvatarIdFromSender()
        if not context in self.requests:
            self.air.writeServerEventMessage('suspicious', avId, 'Player tried to acknowledge the cancel of a friend request that doesn\'t exist!')
            return
        if avId != self.requests[context][0][1]:
            self.air.writeServerEventMessage('suspicious', avId, 'Player tried to acknowledge someone else\'s cancel!')
            return
        if self.requests[context][1] != 'cancelled':
            self.air.writeServerEventMessage('suspicious', avId, 'Player tried to cancel non-cancelled request!')
            return
        del self.requests[context]
